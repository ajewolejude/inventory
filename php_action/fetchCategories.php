...
$sql = "SELECT categories_id, categories_name, categories_active, categories_status, type FROM categories WHERE categories_status = 1";
...
if($result->num_rows > 0) { 
 ...
 while($row = $result->fetch_array()) {
	...
	if($row[4] == 1) {
 		// activate member
 		$typeCategories = "<label class='label label-success'>Asset</label>";
 	} else if ($row[4] == 2) {
 		// deactivate member
 		$typeCategories = "<label class='label label-warning'>Consumable</label>";
 	} else {
		$typeCategories = "<label class='label label-danger'>Others</label>";
	}
