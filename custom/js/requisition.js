...
$(document).ready(function() {
	...
	// top nav bar 
	$("#navRequisition").addClass('active');
	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
			...
			if(orderDate && clientName && reqNo) {
				//alert(orderDate);
	            	...
	            	'<a href="requisition.php?o=add" class="btn btn-default" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> Add New Requisition </a>'+
		...
		manageOrderTable = $("#manageOrderTable").DataTable({
			'ajax': 'php_action/fetchRequisition.php',
			'order': []
		});		
		...
		$("#editOrderForm").unbind('submit').bind('submit', function() {
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && reqNo && paymentStatus) {
...
function printOrder(orderId = null) {
	if(orderId) {		
		...
		$.ajax({
			url: 'php_action/printRequisition.php',
			type: 'post',
		...
		arrayNumber = 0;
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				'<td>'+
					...
					'<input type="text" name="productName[]" id="productName'+count+'" autocomplete="off" required class="form-control" />'+
				'</td>'+
				'<td style="padding-left:20px;"">'+
					'<input type="text" name="rate[]" id="rate'+count+'" autocomplete="off" required  class="form-control" />'+
				'</td style="padding-left:20px;">'+
				'<td style="padding-left:20px;">'+
					'<div class="form-group">'+
					'<input type="number" name="quantity[]" id="quantity'+count+'" required onkeyup="getTotal('+count+')" autocomplete="off" class="form-control" min="1" />'+
					'</div>'+
				'</td>'+
				'<td style="padding-left:20px;">'+
					'<input type="text" name="total[]" id="total'+count+'" required autocomplete="off" class="form-control"  />'+
				'</td>'+
				'<td>'+
...
function paymentOrder(orderId = null) {
	if(orderId) {
		...
		$.ajax({
			url: 'php_action/fetchRequisitionData.php',
			type: 'post',
			data: {orderId: orderId},
			dataType: 'json',
			success:function(response) {				
				...
				// due 
				$("#reqNo").val(response.order[0]);				

				$("#paymentStatus").val(response.order[1]);
